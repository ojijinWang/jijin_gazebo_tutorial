...
int main(int argc, char** argv)
{
	...
	ros::Publisher joint_4_pub = nh_.advertise<std_msgs::Float64>("joint_4_angle_out", 1);
	ros::Publisher joint_5_pub = nh_.advertise<std_msgs::Float64>("joint_5_angle_out", 1);
	ros::Publisher joint_6_pub = nh_.advertise<std_msgs::Float64>("joint_6_angle_out", 1);
	...
	while(nh_.ok())
	{
		if(is_jointState)
		{
			...
			std_msgs::Float64 command_4;
			command_4.data = jointState_in.position[3];
			std_msgs::Float64 command_5;
			command_5.data = jointState_in.position[4];
			std_msgs::Float64 command_6;
			command_6.data = jointState_in.position[5];

			joint_1_pub.publish(command_1);
			joint_2_pub.publish(command_2);
			joint_3_pub.publish(command_3);
			joint_4_pub.publish(command_4);
			joint_5_pub.publish(command_5);
			joint_6_pub.publish(command_6);
		}
		ros::spinOnce();
		loop_rate.sleep();
	}
